...
static iomux_v3_cfg_t const usdhc3_pads[] = {
	...
	IOMUX_PADS(PAD_SD1_CLK__GPIO1_IO20    | MUX_PAD_CTRL(NO_PAD_CTRL)), /* CD */
};
...
#define USDHC3_CD_GPIO	IMX_GPIO_NR(1, 20)
...
{
	if (is_mx6dq()) {
		if (!strcmp(name, "imx6q-icb"))
			return 0;
	} else if (is_mx6dqp()) {
		if (!strcmp(name, "imx6qp-icb"))
			return 0;
	} else if (is_mx6dl()) {
		if (!strcmp(name, "imx6dl-icb"))
			return 0;
	}
